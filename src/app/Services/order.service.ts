...
export class OrderService {
    ...
    newOrder(order: Order): Observable<any> {
        return this.httpClient.post<any>("http://localhost:8000/orders/newOrder/", order).pipe(
            catchError(error => {
    ...
    getOrdersByEmail(email: string): Observable<{ orders: any[] }> {  // ✅ Expect an object, not an array
        return this.httpClient.get<{ orders: any[] }>(`http://localhost:8000/orders/getOrder/${email}`).pipe(
            catchError(error => {
